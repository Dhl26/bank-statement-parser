...
def extract_rbi_transactions(text: str) -> pd.DataFrame:
    rows = []
...
# ---------------------------
def main():
    st.set_page_config(page_title="RBI Bank Statement Parser", layout="wide")
    st.title("🏦 RBI Bank Statement Parser")

    uploaded_file = st.file_uploader("Upload RBI Bank Statement PDF (text-based)", type=["pdf"])
    ...
    if uploaded_file:
        ...
        acct = rbi_bank(all_text)
        st.table(pd.DataFrame(acct.items(), columns=["Field", "Value"]))

        # Transactions
        st.subheader("💰 Transactions")
        txns = extract_rbi_transactions(all_text)
